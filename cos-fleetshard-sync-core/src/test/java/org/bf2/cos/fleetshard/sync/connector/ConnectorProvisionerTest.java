...
public class ConnectorProvisionerTest {
    ...
    void createResources() {
        ...
        assertThat(sc.getValue()).satisfies(val -> {
            assertThat(val.getMetadata().getName())
                .startsWith("c-")
                .endsWith("-s-" + deployment.getMetadata().getResourceVersion());
            ...
            assertThat(val.getData())
                .containsKey(Constants.SECRET_ENTRY_KAFKA)
                .containsKey(Constants.SECRET_ENTRY_CONNECTOR);

            var kafkaNode = Secrets.get(val, Constants.SECRET_ENTRY_KAFKA, KafkaConnectionSettings.class);
            ...
            var connectorNode = Secrets.get(val, Constants.SECRET_ENTRY_CONNECTOR);
            assertThatJson(Secrets.get(val, Constants.SECRET_ENTRY_CONNECTOR))
                .inPath("connector.foo").isEqualTo("connector-foo");
            assertThatJson(connectorNode)
                .inPath("kafka.topic").isEqualTo("kafka-foo");

            var metaNode = Secrets.get(val, Constants.SECRET_ENTRY_META);
    ...
    void updateAndCreateResources() {
        ...
        assertThat(sc.getValue()).satisfies(val -> {
            assertThat(val.getMetadata().getName())
                .isEqualTo("old-cid-s-" + newDeployment.getMetadata().getResourceVersion());
