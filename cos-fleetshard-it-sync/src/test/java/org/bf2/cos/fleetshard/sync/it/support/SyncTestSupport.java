...
public class SyncTestSupport {
    ...
    public static ConnectorDeployment createDeployment(long deploymentRevision) {
        return createDeployment(
            ...
            () -> {
                ObjectNode answer = Serialization.jsonMapper().createObjectNode();
                answer.put("connector_type", "sink");
                answer.put("connector_image", "quay.io/mcs_dev/aws-s3-sink:0.0.1");
                answer.withArray("operators").addObject()
    ...
    @BeforeEach
    public void setUp() {
        this.fleetShardClient = new FleetShardClient(ksrv.getClient(), clusterId, namespace, Duration.ZERO);
    }
}
