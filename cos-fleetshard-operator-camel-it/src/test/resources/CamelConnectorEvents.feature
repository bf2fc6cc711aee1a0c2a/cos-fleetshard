Feature: Camel Connector Status
  ...
  Scenario: klb get edited outside operator control
    Given a Connector with:
      | connector.type.id           | log_sink_0.1                    |
      | desired.state               | ready                           |
      | kafka.bootstrap             | kafka.acme.com:443              |
    ...
    When the klb path "$.spec.integration.configuration[?(@.type == 'env')]" is set to json:
      """
      {
        "type":"env",
        "value": "QUARKUS_LOG_CONSOLE_JSON=true"
      }
      """
    Then the klb exists
    Then the klb path "$.spec.integration.configuration[?(@.type == 'env')]" matches json:
      """
      [{
        "type":"env",
        "value": "QUARKUS_LOG_CONSOLE_JSON=false"
      }]
      """