...
public class ProcessorDeploymentProvisioner {
    ...
    private ManagedProcessor createProcessor(String uow, ProcessorDeployment deployment, HasMetadata owner) {
        ...
        ManagedProcessor processor = fleetShard.getProcessor(deployment).orElseGet(() -> {
            LOGGER.info(
            ...
            ManagedProcessor answer = new ManagedProcessor();
        ...
        if (kafkaConnectionSettings != null) {
            processor.getSpec().setKafka(new KafkaSpec(
        ...
        LOGGER.info("Provisioning processor namespace: {}, name: {}, revision: {}",
            processor.getMetadata().getNamespace(),
            processor.getMetadata().getName(),
            processor.getSpec().getDeploymentResourceVersion());
